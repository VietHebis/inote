...
if (isset($_POST['ac']))
{
    ...
    else if ($ac == 'edit')
    {
        ...
        ";
            ...
            <script>
                location.href = 'index.php?ac=edit_note&&id=".$id_edit_note."';
            </script>
        ";
        }
        // Ngược lại không
...
// Nếu hành động là delete
    ...
    else if ($ac == 'remind')
    {
        // Nhận dữ liệu và gán vào các biến đồng thời xử lý chuỗi
        $id_edit_note = $db->real_escape_string(@$_POST['id_edit_note']);
        $date_remind = $db->real_escape_string(@$_POST['date_remind']);

        $id_edit_note = trim(htmlentities($id_edit_note));
        $date_remind = trim(htmlentities($date_remind));

        // Các biến chứa code JS về thông báo
        $show_alert = "<script>$('#modalRemind .alert').removeClass('hidden');</script>";
        $hide_alert = "<script>$('#modalRemind .alert').addClass('hidden');</script>";
        $success_alert = "<script>$('#modalRemind .alert').attr('class', 'alert alert-success');</script>";
        ...
        ";
            ...
            // Hiển thị thông báo và tải lại trang
            echo $show_alert.$success_alert." Đã tạo nhắc nhở
            <script>
                location.href = 'index.php?ac=edit_note&&id=".$id_edit_note."';
            </script>
        ";
